...
> Question: How could we make sure that we moved? Answer: We can type pwd and press enter to confirm we see the test_directory output
...
would we do that? To save the output of the 'curl' command to a file called 'data.txt' I used the curl command and assigned it to the data.txt file by using the greater than symbol. curl (link) > data.txt
